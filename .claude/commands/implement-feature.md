...
2. **推奨事項 セクション**: 優先度ラベルに従って抽出
   ...
   - `### 優先度: 中（修正推奨）` 配下の項目 → 中
   - `### 優先度: 低（検討事項）` 配下の項目 → 低
3. **次のアクション**: 未完了（`[ ]`）の項目 → 項目内に優先度ラベル（例: `（高優先度）`、`（中優先度）`、`（低優先度）`）がある場合はそれに従う。ラベルがない場合は中
...
**既存の `tasklist.md` がある場合:** 末尾に「レポート指摘・推奨事項の対応」セクションを追記
...
- FAIL項目だけでなく、推奨事項（推奨対応・改善提案）も対応タスクとして抽出する
- 可能な限り対象ファイルパスを含める
...
1. **テストファイルの配置**
   ...
   例:
   src/processors/translator.py → tests/test_translator.py
   src/collectors/arxiv.py → tests/test_arxiv.py
   ```
