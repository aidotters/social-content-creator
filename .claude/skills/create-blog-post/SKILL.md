...
3. **タイプに応じた情報収集**を行う
   - `weekly-ai-news`:
     ...
     3. Notion API経由でMedium Daily Digest取得（`date_from`/`date_to`で明示的に期間指定）
        ...
        "
        ```
     4. WebSearchで最新AIニュースを検索（**グローバルと日本国内の両方**を検索する）
        - グローバル: `AI news highlights {対象期間} {年}`
        - 日本国内: `日本 AI ニュース {年}年{月}月 第{週}週`
     5. **日本国内ニュースの重点収集**: 収集したNotionデータから日本関連ニュースを明示的に抽出する。以下のキーワードでフィルタリングし、国内ニュースの候補を十分に確保すること:
        - 企業名: ソフトバンク、トヨタ、NTT、NEC、ソニー、富士通、楽天、日立、パナソニック、三菱、サイバーエージェント、Preferred Networks、ABEJA、Sakana AI、みずほ、SMBC、DeNA、メルカリ、KDDI、ドコモ 等
        - 機関名: デジタル庁、経産省、総務省、文科省、NEDO、産総研、東大、理研 等
        - 一般キーワード: 日本、国内、国産、円、政府、閣議、法案 等
     6. **日付バリデーション**: 収集したニュースの `published_date` を確認し、対象期間外（`date_from` 〜 記事作成日）のニュースは記事に含めないこと。対象期間外のニュースが混入していた場合は必ず除外する
     7. **日本ニュース構成比率**（必須ルール）:
        - 主要ニュース（8〜10件）のうち、**最低3割（3件以上）、最大7割（7件以下）は日本国内のニュース**を含めること
        - 日本企業の海外展開や、海外企業の日本市場に関するニュースも「日本関連」としてカウントする
        - 業界動向・トレンドのセクションでも、日本国内の動向に1テーマ以上を割くこと
        - 日本のニュースが少ない週でも、最低3件は確保する。Notionデータに不足がある場合はWebSearchで `日本 AI {キーワード} {年}年{月}月` を追加検索して補う
     8. **ソース選定基準**（情報の信頼性を担保するため、以下を厳守する）:
