"""生成画像の詰まり具合を測るユーティリティ。

主に使うのは測定側（content_density / content_bbox）。生成した絵が薄いか
どうかを判定し、薄ければ引き直す（src/utils/image_retry.py）ために使う。

trim_to_16x9 は余白を切り落として 16:9 に組み直す処理だが、生成後に切るより
生成段階で詰まった絵を引くほうが良いという判断で、通常のフローには入れて
いない。明示的に呼べば使える。
"""
